...
docker run  -d --rm -e SKIPSYNC=true -v $OLDTAG-vol:/data --name $OLDTAG-vol -p 8081:9392 mitexleo/openvas:$OLDTAG
...
docker run -d -v $OLDTAG-vol:/data -p 9001:9392 --name oldvol-$tag -e SKIPSYNC=true mitexleo/openvas:$tag
